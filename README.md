...
testable. The `graduation` core owns validated Jira sites, credentials,
identities, and login transitions; `graduation-cli` owns external I/O.
...
```bash
npm install --global @treramey/grad
```
...
brew install treramey/tap/grad
```

Or build directly from this repository:

```bash
cargo install --path crates/graduation-cli
```
...
`GRAD_REDUCED_MOTION=1` to replace moving login effects with short fades.
...
configuration path is `~/.grad/config.json`; override it with `--config` or
`GRAD_CONFIG`.
...
└── graduation-cli/  # Clap, Crossterm, Ratatui, process errors, and terminal lifecycle
```
