...
use shared::error::{CliError, MachineError};
...
async fn main() -> ExitCode {
    ...
    let cli = match Cli::try_parse() {
        ...
        Err(error) => {
            ...
            let _ = error.print();
            return ExitCode::from(u8::try_from(exit_code).unwrap_or(shared::error::EXIT_USAGE));
        }
    };
...
async fn run(cli: Cli) -> Result<(), CliError> {
    match cli.command {
        Command::Auth(args) => {
            let path = cli.config.unwrap_or(shared::config::config_path()?);
            match args.command {
